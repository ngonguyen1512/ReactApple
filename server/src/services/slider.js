...
export const getAllSliderService = () => new Promise(async (resolve, reject) => {
    try {
        const response = await db.Slider.findAll({ raw: true });
        resolve({
            err: response ? 0 : 1,
...
export const createSliderService = ({ name, url, state }) => new Promise(async (resolve, reject) => {
    try {
        const response = await db.Slider.create({ name, url, state });
        resolve({
            err: response ? 0 : 2,
...
export const deleteSliderService = (id) => new Promise(async (resolve, reject) => {
    try {
        const whereClause = {};
        whereClause.id = id;
        const response = await db.Slider.findOne({ where: whereClause, });
...
export const updateSliderService = ({ id, name, url, state }) => new Promise(async (resolve, reject) => {
    try {
        ...
        const response = await slider.update({ name, url, state });
        resolve({
            err: response ? 0 : 2,
