...
export const getAllSamplesService = () => new Promise(async (resolve, reject) => {
    try {
        const response = await db.Sample.findAll({
            raw: true,
            include: [{ model: db.Category, as: 'categoriesmodel', attributes: ['name'] }],
        });
        resolve({
            err: response ? 0 : 1,
...
export const getTypeSamplesService = (query) => new Promise(async (resolve, reject) => {
    try {
        ...
        const response = await db.Sample.findAll({
            raw: true,
            where: whereSample,
            include: [{ model: db.Category, as: 'categoriesmodel', attributes: ['name'], where: whereClause, },],
        });
        resolve({
            err: response ? 0 : 1,
            msg: response ? 'OK' : 'Failed to get category',
